...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#maingrid4").ligerGrid({
                columns: [
                    ...
                    {
                        display: '编号', name: 'receivable_no', width: 180, render: function (item) {
                            var html = "<a href='javascript:void(0)' onclick=view('receivable','" + item.id + "')>";
                    ...
                    {
                        display: '所属客户', name: 'cus_name', width: 200, align: 'left', render: function (item) {
                            var html = "<a href='javascript:void(0)' onclick=view('customer','" + item.Cus_id + "')>";
                            if (item.cus_name)
                                html += item.cus_name;
                            html += "</a>";
                            return html;
        ...
        function toolbar() {
            $.get("toolbar.GetSys.xhd?mid=finance_receivable&rnd=" + Math.random(), function (data, textStatus) {
                var data = eval('(' + data + ')');
                //alert(data);
                var items = [];
                var arr = data.Items;
        ...
        function doserch() {
            ...
            manager._setUrl("Finance_Receivable.grid.xhd?" + serchtxt);
        }
        function doclear() {
        ...
        function add() {
            f_openWindow("finance/receivable_add.aspx?customer_id="+getparastr("customer_id"), "新增应收", 730, 450, f_save);
        }
        ...
        function edit() {
            ...
            if (row) {
                f_openWindow('finance/receivable_add.aspx?id=' + row.id, "修改应收", 730, 450, f_save);
            } else {
        ...
        function del() {
            ...
            if (!row) {
                ...
                $.ligerDialog.confirm("数据删除无法恢复，确定删除？", function (yes) {
                    if (yes) {
                        $.ajax({
                            url: "Finance_Receivable.del.xhd", type: "POST",
        ...
        function f_save(item, dialog) {
            ...
            if (issave) {
                ...
                $.ajax({
                    url: "Finance_Receivable.save.xhd", type: "POST",
                    data: issave,
                    dataType: "json",
                    success: function (result) {
                        top.$.ligerDialog.closeWaitting();
    ...
    <div class="az" style="padding-left: 10px;">
        <form id='serchform'>
           <table style='width: 730px'>               
                <tr>
                    <td>
                        <div style='width: 60px; text-align: right; float: right'>应收时间：</div>
                    </td>
                    <td>
                    ...
                    <td>
                        <div style='width: 60px; text-align: right; float: right'>客户：</div>
                    </td>
                    <td>
                        <input id='T_Customer' name="T_Customer" type='text' ltype='text' ligerui='{width:120}' />
