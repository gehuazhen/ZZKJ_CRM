...
<head>
    ...
    <script type="text/javascript">
        ...
        function loadForm(oaid) {
            $.ajax({
                ...
                success: function (result) {
                        ...
                        fields: [
                            {
                                ...
                                rows: [
                                    [
                                     ...
                                     [
                                        { display: "任务类别", name: "T_task_type", type: "select", options: "{width:180, url: 'Sys_Param.combo.xhd?type=task_type',value:'" + obj.task_type_id + "'}", validate: "{required:true}" },
                                    ...
                                    [
                                        { display: "任务状态", name: "T_status", type: "select", options: "{width:180,data:[{id:0,text:'进行中'},{id:1,text:'已完成'},{id:2,text:'已终止'}],value:0 }", validate: "{required:true}" },
                                        { display: "优先级", name: "T_priority", type: "select", options: "{width:180,data:[{id:0,text:'高'},{id:1,text:'中'},{id:2,text:'低'}],value:1 }" }
                                    ],
                                    [
                                        { display: "任务说明", name: "T_content", type: "textarea", cols: 74, rows: 8, width: 470, cssClass: "l-textarea", initValue: obj.task_content }
                                    ]
                                ]
                            }
                        ]
                    ...
                    obj.priority_id && $("#T_priority").ligerGetComboBoxManager().selectValue(obj.priority_id)
                    obj.task_status_id && $("#T_status").ligerGetComboBoxManager().selectValue(obj.task_status_id);

                    $("#T_status").ligerGetComboBoxManager()._setReadonly(1);
                }
            });
        }
        ...
        function f_selectCustomer() {
            $.ligerDialog.open({
                title: '选择客户', width: 650, height: 300, url: '../../crm/customer/getcustomer.aspx', buttons: [
                    { text: '清空', onclick: fillcus },
