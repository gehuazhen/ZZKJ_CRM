...
(function ($) {
    ...
    $.ligerManagers.AutoForm = function (options, po) {
        this.options = options;
        this.po = po;
    };

    $.ligerManagers.AutoForm.prototype = {
        getType: function () {
            return 'AutoForm';
        },
        idPrev: function () {
    ...
    $.fn.ligerAutoForm = function (p) {
        this.each(function () {
            p = $.extend({}, $.ligerDefaults.AutoForm, p || {});
            var po = {};
            var g = new $.ligerManagers.AutoForm(p, po);
    ...
    $.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function () {
            ...
            $("> .l-accordion-header", g.accordion).each(function () {
                g.headerHoldHeight += $(this).height() + 1;//因为有1px的边框
            });
    ...
    $.ligerui.controls.Button.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            ...
            g.button.hover(function () {
                ...
                g.button.addClass("l-btn-over");
            }, function () {
                if (p.disabled) return;
                g.button.removeClass("l-btn-over");
            });
        ...
        _setIcon: function (url) {
            var g = this;
            if (!url) {
                g.button.removeClass("l-btn-hasicon");
                g.button.find('img').remove();
            } else {
                g.button.addClass("l-btn-hasicon");
        ...
        selectValueByTree: function (value) {
            ...
            if (value != null) {
                ...
                $(valuelist).each(function (i, item) {
                    g.treeManager.selectNode(item.toString(), true);
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        updateSelectBoxPosition: function () {
            ...
            if (p.absolute) {
                ...
                //}
                if (contentHeight > g.dateeditor.height()) {
                    if ((contentHeight - Number(g.text.offset().top + 1 + g.text.outerHeight())) > g.dateeditor.height()) {
                        //控件下部大于下拉框,向下显示 
                        g.dateeditor.css({ left: g.text.offset().left, top: g.text.offset().top + 1 + g.text.outerHeight() });
                    }
                    else if (Number(g.text.offset().top) > g.dateeditor.height()) {
                        //控件上部部大于下拉框,向上显示
                        g.dateeditor.css({ left: g.text.offset().left, top: g.text.offset().top - 1 - g.dateeditor.height() });
                    }
                    else {
                        //控件上下部都小于下拉框，但document大于下拉框，则在页面中间显示
                        g.dateeditor.css({ left: g.text.offset().left, top: 0 });
                    }
                }
                else {
                    //document小于控件，则向上显示，以便能点确定按钮
                    g.dateeditor.css({ left: g.text.offset().left, top: g.text.offset().top - 1 - g.dateeditor.height() });
                }
...
(function ($) {
    ...
    //dialog 图片文件夹的路径 预加载
    $.ligerui.DialogImagePath = "../../lib/ligerUI/skins/Gray2014/images/win/";
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        applyWindowMask: function () {
            ...
            g.dialog.WindowMask.css({ zIndex: p.zindex });
        },
        ...
        recover: function () {
            ...
            if (g.winmax) {
                ...
                if (p.slide) {
                    ...
                    g.dialog.content.animate({ height: g._height - g.dialog.buttons.outerHeight() }, p.slide, function () {
                        g._onReisze();
                    });
                }
                else {
    ...
    $.ligerDialog.waitting = function (title, zindex) {
        ...
        return $.ligerDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false, zindex: zindex });
    };
    ...
    $.ligerDialog.success = function (content, title, onBtnClick, zindex) {
        return $.ligerDialog.alert(content, title, 'success', onBtnClick, zindex);
    };
    $.ligerDialog.error = function (content, title, onBtnClick, zindex) {
        return $.ligerDialog.alert(content, title, 'error', onBtnClick, zindex);
    };
    $.ligerDialog.warn = function (content, title, onBtnClick, zindex) {
        return $.ligerDialog.alert(content, title, 'warn', onBtnClick, zindex);
    };
    $.ligerDialog.question = function (content, title, zindex) {
        return $.ligerDialog.alert(content, title, 'question', null, zindex);
    };


    $.ligerDialog.prompt = function (title, value, multi, callback, zindex) {
...
(function ($) {
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        setCheck: function (rowParm) {
            var po = this.po, g = this, p = this.options;
            var row = $("tr[growid=" + rowParm + "]");
            //alert($(row).attr("id"));
            //alert(JSON.stringify( g.records))
            //row.addClass("l-checked");
            g.select({ nodeType: 1, id: $(row).attr("id") });
        },
        //add function
        setUnCheck: function (rowParm) {
            ...
            g.unselect({ nodeType: 1, id: $(row).attr("id") });
        },
        //addfunction()
        ...
        */
        ...
        getCurrentData:function()
        {
            ...
            if (this.deletedRows) {
                $(this.deletedRows).each(function () {
                    var o = $.extend(true, {}, this);
                    removeByValue(data,g.formatRecord(o, false));
                });
            }
            ...
            if (column.totalSummary) {
                ...
                if (column.totalSummary.render) {
                    ...
                    if (isExist('sum_money'))
                        totalsummaryArr.push("<div style='text-align:right;'>￥" + toMoney(info.sum.toFixed(0)) + "</div>");
                }
            }
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _onResize: function () {
            ...
            if (g.left) {
                ...
                g.left.content.height(g.middleHeight - 32);
            }
            if (g.right) {
                g.rightCollapse.height(g.middleHeight);
                g.right.height(g.middleHeight);
                g.right.content.height(g.middleHeight - 32);
                //set left
                g.rightLeft = 0;
    ...
    $.ligerui.controls.Panel.ligerExtend($.ligerui.core.UIComponent, {
        ...
        toggle: function () {
            ...
            if (toggle.hasClass("l-panel-header-toggle-hide")) {
                ...
                g.panel.find(".l-panel-content:first").slideDown("fast");
            } else {
                toggle.addClass("l-panel-header-toggle-hide");
                toggle.attr("title", p.expandMessage);
                g.panel.find(".l-panel-content:first").slideUp("fast");
            }
